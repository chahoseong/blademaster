...
// 쓰러짐(State.Dead)이 붙는 순간 발동한다(OwnedTagAdded — 태그가 떨어져도 취소되지 않는다).
// 설정 시간만큼 기다렸다가 초기화 GE를 다시 적용하고 State.Dead를 없앤다. GA_Hit은 죽음을 판정하고
// State.Dead를 붙이는 것까지만 하고 부활은 완전히 모른다 — 태그로만 연결되는 구조다.
